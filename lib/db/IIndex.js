...
const DBMinerva = function(url,options) {
    ...
    function connectMethods(name,collection,insert_mnp,update_mnp) {
        return {
            insert: (data,options = {}) => {
                    ...
                    co(function* () {
                        if(Array.isArray(data)) {
                            data = data.map((element) => {
                                ...
                                setNextPK(name);
                                element = insert_manipulation(element);
                                return element;
                            });
                        } else {
                            data['_id'] = pk[name].toString(16);
                            setNextPK(name);
                            data = insert_manipulation(data);
                        }
    ...
    this.connect = (url) => {
        co(function* () {
            ...
            self.profiles = connectMethods('profiles',profiles);
    ...
    this.connectProfile = (username) => {
        co(function* () {
            ...
            self.accounts = connectMethods('accounts',accounts);
            ...
            self.groups = connectMethods('groups',groups);
