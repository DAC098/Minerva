...
const DataHandler = function DataHandler(debug) {
    ...
    this.data = {
        ...
        clear: () => {
            try {
                current_data = Object.assign({},templates('data'));
                assert.deepStrictEqual(current_data,templates('data'));
                logger('data has been cleared');
                return true;
            } catch(e) {
                logger(`error when clearing data: ${e.message}`);
                return false;
            }
        }
    }
    ...
    this.group = {
        ...
        addTo: (group_pk,account_pk) => {
            if(account_pk in current_data._accounts) {
                ...
                if(desired_group) {
                    ...
                    if(result) {
                        logger(`account: ${account_pk} added to group: ${group_pk}`);
        ...
        removeFrom: (group_pk,account_pk) => {
            if(account_pk in current_data._accounts) {
                ...
                if(desired_group) {
                    var result = desired_group.ref.remove((element) => {
                        return parseInt(account_pk,parse_value) === parseInt(element,parse_value);
                    });
                    if(result) {
                        logger(`account: ${account_pk} removed from group: ${group_pk}`);
    ...
    this.account = {
        insert: (account_data) => {
            ...
            var found = checkObject(account_data,templates('account'));
            ...
            if(found.valid) {
                ...
                if(account_data.group) {
                    self.group.addTo(account_data.group,new_pk);
                }
        ...
        update: (account_pk,new_data) => {
            var exists = account_pk in current_data._accounts;
            var found = checkObject(new_data,templates('account'));
            if(found.valid && exists) {
                var current_account = current_data._accounts[account_pk];
                if(current_account.group !== '') {
                    self.group.removeFrom(current_account.group,account_pk);
                }
                if(new_data.group && new_data.group !== '') {
                    self.group.addTo(new_data.group,account_pk);
                }
                if(new_data) {
                ...
                fillMissingKeys(new_data,current_account);
                current_data._accounts[account_pk] = new_data;
                logger(`account: ${account_pk} updated`);
                dispatch('account','update',account_pk);
                return account_pk;
            } else {
                var str = "";
                str += (!found.valid) ? `unexpected key value pairs ${found.keys}, ` : '';
                str += (!exists) ? `account: ${account_pk} does not exists, ` : '';
                str += "account update failed";
                logger(str);
            }
        },
        remove: (account_pk) => {
            if(account_pk in current_data._accounts) {
                ...
                current_data._accounts[account_pk] = null;
        ...
        find: (account_pk) => {
            if(account_pk) {
                logger(`searching for account: ${account_pk}`);
                if(account_pk in current_data._accounts) {
                ...
                logger('returning all accounts found');
                var rtn = [];
